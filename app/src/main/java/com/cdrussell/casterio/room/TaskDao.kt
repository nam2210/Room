...
interface TaskDao {
    ...
    @Transaction
    @Query("SELECT Task.id as task_id, Task.completed as task_completed, Task.title as task_title, AssignedTask.user, User.* FROM" +
            " Task LEFT OUTER JOIN AssignedTask on Task.id = AssignedTask.task" +
            " LEFT OUTER JOIN User on AssignedTask.user = User.id")
    fun getAllWithAssignedUsers(): LiveData<List<TaskUserPair>>

    @Query("SELECT Task.id as task_id, Task.completed as task_completed, Task.title as task_title, AssignedTask.user, User.* FROM" +
            " Task LEFT OUTER JOIN AssignedTask on Task.id = AssignedTask.task LEFT OUTER JOIN User on AssignedTask.user = User.id" +
